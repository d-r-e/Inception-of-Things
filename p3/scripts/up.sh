...
if [ -z "$(k3d cluster list | grep iot)" ];then
    ...
    k3d cluster create  --port 80:80@loadbalancer iot --agents 1 --servers 1 --wait
...
while ! kubectl get pods -n kube-system -l k8s-app=metrics-server -o 'jsonpath={..status.conditions[?(@.type=="Ready")].status}' | grep -q "True"; do
